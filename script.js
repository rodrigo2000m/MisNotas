...
function initFunction(){
    ...
    let categoryView=JSON.parse(localStorage.getItem('categoryView'))
...
function deleteCategory(i){
    ...
    //Borro las notas con la categoria
    //Se aplicaron 2 for debido a que cambia la longitud del array
    for(let j=0; j<notes.length; j++){
        for(let j=0; j<notes.length; j++){
            if(notes[j].category===categories[i]){
                notes.splice(j,1);
            }
        }
    }
...
function showNotes(view){
    ...
    let notes=JSON.parse(localStorage.getItem('notes'));
    //Filtro
    if(categoryView!="allNotes"){
    notes=notes.filter(nota => nota.category===categoryView)
    }
    let htmlContentToAppend='';
    for(let i=0;i<notes.length;i++){
        //Filtro por categoria
            htmlContentToAppend+=`
            <div id="note${i}" class="card m-2 p-2 ${view} align-self-start myNotes" data-bs-toggle="modal" data-bs-target="#myModal${i}">
